...
module.exports = (pool) => {
    ...
    router.get('/fetchAdminData', async (req, res) => {
        try {
            ...
            const activeUsersResult = await pool.query`SELECT COUNT(*) as activeUsersCount FROM Users WHERE loggedIn = 1`;
            ...
            // Check system status
            const databaseConnected = await pool.connect();
    ...
    router.post('/tutorAccountCode', async (req, res) => {
        ...
        try {
            const lastGen = await pool.query`SELECT lastGeneratedAt FROM VerificationCode WHERE schoolID = ${schoolID}`;
